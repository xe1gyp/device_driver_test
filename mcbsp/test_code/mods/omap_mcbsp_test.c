			...
			int *eof, void *data)
{
	...
	p += sprintf(p, "================================================\n\n");
	p += sprintf(p, "McBSP ID                                 : "
						"%8d \n", test_mcbsp_id);
	p += sprintf(p, "Buffer size (bytes)                      : "
						"%8d \n", buffer_size);
	p += sprintf(p, "Number of transfers                      : "
						"%8d \n", no_of_trans);
	p += sprintf(p, "No. of buffers transmitted               : "
						"%8d \n", no_of_tx);
	p += sprintf(p, "No. of buffers received                  : "
						"%8d \n", no_of_rx);
	p += sprintf(p, "Sampling Rate (frequency, hertz)         : "
						"%8d \n", sample_rate);
	p += sprintf(p, "Phase [1=Single, 2=Dual]                 : "
						"%8d \n", phase);
	p += sprintf(p, "CLKR_Polarity [1=Rising, 2=Falling]      : "
						"%8d\n", clkr_polarity);
	p += sprintf(p, "FSR_Polarity  [0=High, 1=Low]            : "
						"%8d\n", fsr_polarity);
	p += sprintf(p, "CLKX_Polarity [1=Rising, 2=Falling]      : "
						"%8d\n", clkx_polarity);
	p += sprintf(p, "FSX_Polarity  [0=High, 1=Low]            : "
						"%8d\n", fsx_polarity);
	p += sprintf(p, "Justification [0-RJUST,1-SRJUST,2-LJUST] : "
						"%8d\n", justification);
	p += sprintf(p, "Word Length   [0-8,2-16,5-32]bits        : "
						"%8d\n", word_length);
	p += sprintf(p, "Words Per frame [0-1, 1-2]               : "
						"%8d\n", words_per_frame);
...
create_proc_file_entries(void)
{
	...
	transmission_file = create_proc_entry("transmission", 0644,
							mcbsp_test_dir);
	...
	mcbsp_test_dir->owner = status_file->owner = transmission_file->owner;
	return 0;
...
void omap_mcbsp_send_cb(unsigned short int ch_status, void *arg)
{
	if (no_of_tx <  no_of_trans) {
		if (0 != omap2_mcbsp_send_data(mcbsptest_info.mcbsp_id,
			&mcbsptest_info, mcbsptest_info.tx_buf_dma_phys,
							buffer_size)) {
...
void omap_mcbsp_recv_cb(unsigned short int ch_status, void *arg)
{
	...
	if (no_of_rx <  no_of_trans) {
		if (0 != omap2_mcbsp_receive_data(mcbsptest_info.mcbsp_id,
				&mcbsptest_info, mcbsptest_info.rx_buf_dma_phys,
								buffer_size)) {
			printk(KERN_ERR "McBSP Test Driver: Slave Send "
								"data failedn");
			return;
		}
		no_of_rx++;
	...
	for (k = 0; k < buffer_size; ) {
		if (ptr[k] != 0xAA)
			printk(KERN_ERR "Error!!! in data %d \n", no_of_rx);
		k = k + 1;
	}
#endif
...
{
	/* SRG Reset */
	if (0 != omap2_mcbsp_set_srg(mcbsptest_info.mcbsp_id,
						OMAP2_MCBSP_SRG_DISABLE)) {
		printk(KERN_ERR "McBSP Test Driver: Resetting SRG failed\n");
		return -1;
	}

	/* FSG Reset */
	if (0 != omap2_mcbsp_set_fsg(mcbsptest_info.mcbsp_id,
						OMAP2_MCBSP_FSG_DISABLE)) {
		printk(KERN_ERR "McBSP Test Driver: Resetting FSG failed\n");
		return -1;
	}

	/* Digital Loop back mode */
	if (0 != omap2_mcbsp_datatxrx_mode(mcbsptest_info.mcbsp_id,
							OMAP2_MCBSP_DLB)) {
		printk(KERN_ERR "McBSP Test Driver: Configuring data "
							"tx/rx mode failed\n");
		return -1;
	}

	/* Configure SRG */
	if (0 != omap2_mcbsp_srg_cfg(mcbsptest_info.mcbsp_id, sample_rate,
					bits_per_sample[word_length],
					OMAP2_MCBSP_SRGCLKSRC_FCLK, 0,
	...
	/* Configure FSG */
	if (0 != omap2_mcbsp_fsync_cfg(mcbsptest_info.mcbsp_id,
					OMAP2_MCBSP_TXFSYNC_INTERNAL,
					OMAP2_MCBSP_RXFSYNC_INTERNAL,
					fsx_polarity, fsr_polarity,
					((bits_per_sample[word_length]*2) - 1),
...
int configure_mcbsp_tx(void)
{
	...
	/* XRST Reset */
	if (0 != omap2_mcbsp_set_xrst(mcbsptest_info.mcbsp_id,
	...
	if (0 != omap2_mcbsp_set_trans_params(mcbsptest_info.mcbsp_id, &tp)) {
		printk(KERN_ERR "McBSP Test Driver: Configuring transfer "
							"params failed\n");
		return -1;
	}
	...
	mcbsptest_info.tx_buf_dma_virt = (void *)
		consistent_alloc(GFP_KERNEL | GFP_DMA | GFP_ATOMIC, buffer_size,
...
int configure_mcbsp_rx(void)
{
	...
	/* RRST */
	if (0 != omap2_mcbsp_set_rrst(mcbsptest_info.mcbsp_id,
	...
	mcbsptest_info.rx_buf_dma_virt = (void *)
	    consistent_alloc(GFP_KERNEL | GFP_DMA | GFP_ATOMIC,
				buffer_size, &mcbsptest_info.rx_buf_dma_phys);
#else
	mcbsptest_info.rx_buf_dma_virt = (void *)
		dma_alloc_coherent(NULL, buffer_size,
			&mcbsptest_info.rx_buf_dma_phys, GFP_KERNEL | GFP_DMA);
...
int start_mcbsp_transmission(void)
{
	...
	if (0 != omap2_mcbsp_receive_data(mcbsptest_info.mcbsp_id,
		...
		printk(KERN_ERR "McBSP Test Driver: Slave Send data failed\n");
		return -1;
	}

	printk(KERN_INFO " Start McBSP Tranmitter \n");
...
int __init omap_mcbsp_init(void)
{
	...
	if (omap2_mcbsp_request_interface(mcbsptest_info.mcbsp_id,
					OMAP2_MCBSP_MASTER,
...
void __exit omap_mcbsp_exit(void)
{
	...
	if (omap2_mcbsp_release_interface(mcbsptest_info.mcbsp_id) != 0) {
		printk(KERN_ERR "McBSP Test Driver: Releasing mcbsp "
							"interface failed\n");
	}
