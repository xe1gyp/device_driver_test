...
static void omap2_spi2_irq_handler(void *arg,struct pt_regs *regs)
{
	static int irq_count =0;
	int ret;
	printk("\nIn SPI2 Wake up Irq Handler\n");	
}


int 
__init test_mcspi_wk_init(void)
{
	int ret;
	unsigned int cphase;
	unsigned int cpol;
	...
	omap2_mcspi_enable_wakeup(&spi_dev2,omap2_spi2_irq_handler,0);
...
static void __exit test_mcspi_wk_exit(void)
{
	if(omap2_mcspi_disable_wakeup(&spi_dev2) != 0){
		printk("\n Disabling wakeup failed \n");
	}

	omap2_mcspi_put(&spi_dev2);
	omap2_mcspi_put(&spi_dev3);
	
	if(omap2_mcspi_release_interface(&spi_dev2) !=0){
		printk("\n releasing 2 failed \n");
	}	
	if(omap2_mcspi_release_interface(&spi_dev3) != 0){
		printk("\n releasing 3 failed \n");
	}
...
module_init(test_mcspi_wk_init);
module_exit(test_mcspi_wk_exit);
