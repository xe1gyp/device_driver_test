/*
 *  aplay_pr.c - plays and records. allows pause and resume.
 *
 ...
 *
 *  Changelog:
 *  25/04/07 .- Added pause - resume capability. Oliver Espinosa.  
 */
...
static void pause_resume_thread(void);
...
int main(int argc, char *argv[])
{
	...
	/* create a thread to get pause, resume */
	tmp_err = pthread_create(&ip_thread,
				NULL,
				pause_resume_thread,
				NULL);
	if (tmp_err || ! ip_thread) {
		fprintf(stdout, "Failed to create the pause,res thread !!!!!!! \n");
	}
...
void playback_go(int fd, size_t loaded, off64_t count, int rtype, char *name)
{
	...
	while (written < count) {
		do {
	     ...
	     if ((volatile int) play_pause) {
		     snd_pcm_pause(handle, 1);
		     while ((volatile int)play_resume == 0) {
			     sleep(1);
		     }
...
void pause_resume_thread(void) {
	char ch;

	fprintf(stdout, "starting the pause_resume_thread block !! \n");
	fprintf(stdout, "Press 'P' to pause, 'R' to resume  \n");
	...
	while (1) {
		...
		if (ch == 'p' || ch == 'P') {
			...
			play_pause = 1;
			sleep(1);
			fprintf(stdout, "Press 'R' to resume  \n");
		}
		if (ch == 'r' || ch == 'R') {
			fprintf(stdout,"\n Resumng the Play \n");
			play_pause = 0;
			play_resume = 1;
			sleep(1);
			fprintf(stdout, "Press 'P' to Pause  \n");
		}
	}
}	
