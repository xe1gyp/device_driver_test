			...
			int *gain, int *ctl_base)
{
	...
	if (argc < 3) {
		fprintf(stdout, "Usage: %s <stream_id> <gain> \n", argv[0]);
		fprintf(stdout, "Or: %s <stream_id> <gain> <ctl_base> ",
			argv[0]);
...
int main (int argc, char **argv ){
	...
	status = ProcessArgs(argc, argv, &stream_id, &gain, &ctl_base);
	if (status)
		return status;
	if (ctl_base)
		ctl_num = (unsigned int)ctl_base;
	fprintf(stdout,
		"Requesting Gain of 0x%x for stream 0x%x \n", gain, stream_id);
	ctl_num += stream_id;
	...
	for (idx = 0; idx < count; idx++) {
		...
		case SND_CTL_ELEM_TYPE_INTEGER:
			tmp = 50;
			snd_ctl_elem_value_set_integer(control, idx, gain);
			break;
		case SND_CTL_ELEM_TYPE_INTEGER64:
			tmp = 0;
...
`	*/
	if ((err = snd_ctl_elem_write(handle, control)) < 0) {
		fprintf(stderr,
			"Control %s element write error: %s\n",
			card, snd_strerror(err));
		return err;
	}
