...
initialSetup () {
	test -d $DEFAULT_MOUNT_POINT2 || mkdir $DEFAULT_MOUNT_POINT2
	$TESTBIN/mke2fs $PARTITION2_ENTRY_DEV1
	mount -t ext2 $PARTITION2_ENTRY_DEV1 $DEFAULT_MOUNT_POINT2
	mount | grep "$DEFAULT_MOUNT_POINT2"
	cd $DEFAULT_MOUNT_POINT2
}

cleanUp () {
	cd /
	umount $DEFAULT_MOUNT_POINT2 && sync
}
