...
{
	int i;
	int elem_count = 0, frame_count = 0;
	unsigned int src, dest;
	int ret = 0;
	...
	/* Init statitistics */
	if ((ret = stat_init("dmadynamic_chain", channels, &stat_id))) {
	...
	if ((ret =
	     omap_request_dma_chain(OMAP_DMA_NO_DEVICE,
				    ...
				    &chain_id, channels, OMAP_DMA_DYNAMIC_CHAIN,
				    transfer_params))) {
	...
	ret =
	    omap_dma_chain_a_transfer(chain_id, src, dest, elem_count,frame_count,
				      (void *)&dma_test[i]);
	if (ret) {
		ERR("DYN omap_dma_chain_a_transfer() failed: %d\n", ret);
		dmatest_cleanup();
		return ret;
	}
#if defined(CONFIG_ARCH_OMAP24XX) || defined(CONFIG_ARCH_OMAP34XX)
