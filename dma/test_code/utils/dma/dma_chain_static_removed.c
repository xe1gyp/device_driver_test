 ...
 * until the rounds number is reached.   The link from a chain transfer 
...
static int __init dma_module_init(void) {
       ...
       if(error){
		...
		/* Create link  from another chain*/
		omap_dma_link_lch(transfers[i].chain_id+2, chain.chain_id);
	   ...
	   /* Create the buffers for each transfer */
		...
		/* Interrupt the link while transfer is ongoing  */
	if( (i == 3) ){		
		omap_dma_unlink_lch(transfers[i].chain_id+2, chain.chain_id);
		printk(" \n\nLink from another chain removed");			
		set_test_passed_chain(0);	
		printk(" \n\n");		
	}	
