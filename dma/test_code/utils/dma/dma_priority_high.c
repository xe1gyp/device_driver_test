 ...
 * buffer size, the first three have low priority and the last one
 * high priority. The transfers are started at the same time from
 * the first to the last one, so the low priority transfers are
 * started first then the high priority one. The expected result is
 * the high priority transfer should finish first.
 *
 * History:
...
/*
 * Checks that the first transfer to finish is actually the last in the
 * transfers array with the highest priority.
 */
static void check_test_passed(void){
     ...
     /* Check the last transfer finished first */
     if(!error){
         int first_transfer = transfer_finished_order[0];
         printk("The transfer id %d finished first\n", first_transfer);
         printk("Expected transfer id %d to finish first\n",
              transfers[TRANSFER_COUNT-1].transfer_id);
         if(first_transfer != transfers[TRANSFER_COUNT-1].transfer_id){
              /* The high priority transfer didn't finish first */
              error = 1;
         }
     }
...
static int __init dma_module_init(void) {
       ...
       for(i = 0; i < TRANSFER_COUNT; i++){
           ...
           if( i == TRANSFER_COUNT - 1){
               ...
               /* Set the other transfers to low priority */
