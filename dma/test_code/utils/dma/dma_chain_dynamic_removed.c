...
void dma_callback_chain(int transfer_id, u16 transfer_status, void *data) {
	...
	/* Create link  from another chain*/
		omap_dma_link_lch(transfer->chained_id, transfer->chain_id+1);	

	/* Removed link  from another chain*/
	if(transfer->chained_id == 3){	
		omap_dma_unlink_lch(transfer->chained_id, transfer->chain_id+1);
		set_test_passed_chain(0);
		printk(" Link from another chain removed\n \n ");
	}
