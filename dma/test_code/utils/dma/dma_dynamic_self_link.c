...
void dma_callback(int transfer_id, u16 transfer_status, void *data) {
	...
	/* Create the buffers for a new transfer */
      // transfers[current_transfer].buffers.buf_size = (1024 * 1024);
       error = create_transfer_buffers(
             &(transfers[current_transfer].buffers));
       if( error ){
           return;
       }
       fill_source_buffer(&(transfers[current_transfer].buffers));

       /* Chain a transfer to the current dynamic chain */
       printk(" Linking transfer %d with it self\n",
                transfer->transfer_id);
       dma_link_lch(&transfers[current_transfer]);
              
       printk(" Linking succeed\n");

       if(error){
           printk(" Error linking %d to itself \n",
                    transfer->transfer_id);
           transfer->data_correct = 0;
           return;
       }
	 printk(" Starting the DMA transfer after linking\n");
	start_dma_transfer(&transfers[current_transfer]);
       transfer->data_correct = 1;
}
