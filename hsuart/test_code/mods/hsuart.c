...
static void omap24xx_hsuart_rx_callback(int lch, u16 ch_status, void *data)
{
...
static void omap24xx_hsuart_tx_callback(int lch, u16 ch_status, void *data)
{
	struct omap24xx_hsuart *si = data;
...
static void omap24xx_hsuart_startup(void)
{
	...
	if (omap24xx_uart_config(uart_no, &uartcfg))
		printk(KERN_ERR "!!! %s !!!! Error ...UART configuration Failed", __FUNCTION__);
}

static int omap24xx_hsuart_tx()
{
	int i;
...
static int omap24xx_hsuart_start()
{
...
static int omap24xx_hsuart_stop()
{
	if (si->tx_status == 0)
		omap24xx_uart_stop_tx(uart_no);
...
static int read_proc_entries(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len = 0;
	...
	if (*procfiletype == 1)
	{
		...
		if (showtxtime == 1)
			p += sprintf(p, "Time taken       %08li sec %08li usec\n", si->tx_sec, si->tx_usec);
...
static int write_proc_entries(struct file *file, const char *buffer, unsigned long count, void *data)
{
	int len, i;
	char val[10];
...
static int create_proc_file_entries(void)
{
	...
	hsuart_dir->owner = status_file->owner = starttx_file->owner = THIS_MODULE;
	return 0;
...
static void omap24xx_uart_irq(u8 iir_data, void *dev_id)
{
	if (iir_data)
	{}
}

static int omap24xx_hsuart_initialize()
{
	...
	uart_txrxisr.uart_tx_dma_callback = (void *) omap24xx_hsuart_tx_callback;
	uart_txrxisr.uart_rx_dma_callback = (void *) omap24xx_hsuart_rx_callback;
	...
	if (bufsize <= 0 || bufsize > 0x100000)
		printk(" ERROR! Buffer Size should be greater than 1 and less than 0x100000\n");
...
static int /*__init */ omap24xx_hsuart_init(void)
{
	omap24xx_hsuart_initialize();
	return 0;
}

static void /*__exit */ omap24xx_hsuart_exit(void)
{
	if (!failed)
	{
