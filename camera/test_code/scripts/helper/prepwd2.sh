...
if [ -z "$STRESS" ]; then
  echo "";echo "Was camera able to preview video in $FORMAT format with $SIZE size and $ROT rotation on $OUT without kernel crash during suspend-suspend-resume?";echo ""
  $WAIT_ANSWER
  ERR=$?
  if [ $ERR -eq 1 ]; then
    echo "FAIL"
