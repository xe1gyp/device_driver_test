...
setup()
{
    ...
    if [ -f "./conf.sh" ]; then
	    ...
	    export TMPBASE=${TESTROOT}/tmp
		  ...
		  export TMPFILE2=${TMPBASE}/tmp2.$POSTFIX
...
usage()
{
    ...
    usage: ./${0##*/} [-z] [-h] [-v] [-d TESTDIR] [-o OUTPUTFILE] [-l LOGFILE] [-T TESTCASE] 
    [-n DURATION ] [-t TMPDIR] [-N] [SCENARIO_NAMES..]
    ...
    -T 		    Special Mode - Execute only one test case of the Scenario
    -N		    Special Mode - Execute only the test cases marked as Sanity tests
...
sanity_check()
{
	...
	cat $TC_SCENARIO/$SCEN|grep -v "#"|sed -e "s/^[  ]*$//g"|sed -e "/^$/d"|grep -e '^[A-Za-z]' -e $TESTCASE > $TMPFILE|| die "Count not create tmp file"
	fi
	if [ -z "$DONT" ]; then
	...
	if [ -z "$EXTENDED_TEST" ]; then
		...
		cat $CMDFILE|grep -v "^[_A-Za-z0-9]*_EXT ">$TMPFILE || die "intermediate file gen failed for extended"
		cat $TMPFILE>$CMDFILE || die "Second intermediate creation failed"
	fi
	if [ -z "$INTERACTIVE" ]; then
		#Remove the test cases which require user interaction
		cat $CMDFILE|grep -v "^[_A-Za-z0-9]*_INT ">$TMPFILE || die "intermediate file gen failed for interactive"
		cat $TMPFILE>$CMDFILE || die "Second intermediate creation failed"
	fi
	if [ $SANITY ]; then
		#Remove the test cases which are not part of a Sanity test
		info "EXECUTING ONLY TEST CASES PART OF A SANITY TEST"
		cat $CMDFILE|grep -e 'SAN' -e 'COMMENT' >$TMPFILE || die "intermediate file gen failed for sanity"
                cat $TMPFILE>$CMDFILE || die "Second intermediate creation failed"
	fi
...
{
   count=0
    while getopts zx:Sd:qt:po:l:T:vn:hs:EIN arg
    do  case $arg in
	T)
	   TESTCASE=${OPTARG} ;;
        d)
