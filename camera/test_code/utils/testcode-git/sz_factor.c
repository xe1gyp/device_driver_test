...
{
	printf("streaming  [camDevice] [pixelFormat] [<sizeW> <sizeH>] [(vid)] [<count>] [zoomFactor]\n");
	printf("\tTo start streaming capture of 500 frames\n");
	printf("   [camDevice] 1: Micron sensor 2: OV Sensor\n");
	printf("   [pixelFormat] set the pixelFormat to use. \n\tSupported: YUYV, UYVY, RGB565, RGB555, RGB565X, RGB555X, RAW10 \n");
	printf("   [sizeW] Set the video width\n");
	printf("   [sizeH] Set the video heigth\n");
	printf("\tOptionally size can be specified using standard name sizes(VGA,PAL,etc)\n");
	printf("\tIf size is NOT specified QCIF used as default\n");
	printf("   [vid] is the video pipeline to be used. Valid vid is 1 (default) or 2\n");
	printf("   [count] amount of frames to be displayed/saved\n");
	printf("   [zoomFactor] Please enter the value of zoofactor between 10 and 40\n");
}
...
int main (int argc, char *argv[])
{
  ...
  if (argc > index){
    ...
    else
    {
      ...
      if (argc > (index))
      {
        ret = cam_ioctl(fd,pixelFmt,argv[index-1], argv[index]);
        if (ret < 0)
        {
        usage();
        return -1;
  ...
  {
      printf("Setting pixel format and video size with default values\n");
	...
	if (argc > index) {
		...
		if (argc > index)
			...
			index++;
			        ...
			        if (argc > index) {
			                zoomFactor = atoi(argv[index]);printf("\n Zoom Factor = %d\n",zoomFactor);
			                if(zoomFactor < 10 || zoomFactor >40) {
			                        printf("ERROR: Zoom Factor Not Supported Value must be between"
			                                                                " 10-40\n");
			                        printf("argv[%d]=%s\n",index,argv[index]);
			                        usage();
			                        return 0;
...
//                 printf("\nZoomIn\n");
//printf("\n using %d \n",zoomFactor);
 //                 ret= crop(fd,(cformat.fmt.pix.width - (cformat.fmt.pix.width * 10)/zoomFactor) / 2,(cformat.fmt.pix.height - (cformat.fmt.pix.height * 10)/zoomFactor) / 2,(cformat.fmt.pix.width * 10)/zoomFactor, (cformat.fmt.pix.height * 10)/zoomFactor);
	...
	printf("Requesting %d buffers of type %s\n", creqbuf.count,
		(memtype == V4L2_MEMORY_USERPTR)?"V4L2_MEMORY_USERPTR":"V4L2_MEMORY_MMAP");
    ...
    if (memtype == V4L2_MEMORY_USERPTR) {
			if (vid == 0) {
				...
				if (cbuffers[i].length & 0xfff) {
					cbuffers[i].length = (cbuffers[i].length & 0xfffff000) + 0x1000;
				}
	...
	//OJV. For OV3640 Sensor testing uncomment the next line
	//printf("\nSleep enabled OV3640\n");sleep(4);
  ...
  /*if (argc >= (index + 1)) {
	...
	while (i < 300) {
		...
		while (ioctl(fd, VIDIOC_DQBUF, &cfilledbuffer) < 0) {
			perror ("VIDIOC_DQBUF");
			if (vid != 0) {
				while (ioctl(screen_info.fd, VIDIOC_QBUF, &vfilledbuffer) < 0) {
					perror ("VIDIOC_QBUF***");
				}
			}
			else {
		...
		if (fd_save > 0) {
			...
			write(fd_save, src_start, cformat.fmt.pix.width*cformat.fmt.pix.height*2);
			...
			if (vid == 0) {
				...
				rotate_image(src_start, cformat.fmt.pix.width, cformat.fmt.pix.height,
