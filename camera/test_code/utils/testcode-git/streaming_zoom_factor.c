...
static void usage(void)
{
	...
	printf("   [zoomFactor] zoom factor to be used during all "
			"captured frames \n      Supported values: 10-40\n");
...
/****************************************************************************/
	...
	/* capture 500 frames or when we hit the passed nmuber of frames */
	cfilledbuffer.type = creqbuf.type;
	vfilledbuffer.type = vreqbuf.type;
	i = 0;
	vfilledbuffer.index = -1;
	
	/*Changing zoom factor to be used during all the frames*/
	printf("Setting zoom factor to be used during all captured frames\n");
	...
	while (i < 500) {
		...
		/* this compile option allow us to write video to a file */
			...
			/* De-queue the previous buffer from video driver */
				...
				/* Queue the new buffer to video driver 
								for rendering */
