...
#define case_scenario_f(tlog, str, args...) \
    snprintf(tlog->log_buff, LOG_BUFF_SIZE, str, args); \
    case_scenario(tlog, (const char*) tlog->log_buff)    

#define case_description_f(tlog, str, args...) \
    snprintf(tlog->log_buff, LOG_BUFF_SIZE, str, args); \
    case_description(tlog, (const char*) tlog->log_buff)    

#define case_validation_f(tlog, str, args...) \
    snprintf(tlog->log_buff, LOG_BUFF_SIZE, str, args); \
    case_validation(tlog, (const char*) tlog->log_buff)    
...
typedef enum {ERRORL, INFO, PASS, FAIL, WARN, START, END, CASE_START, CASE_PASS, CASE_FAIL, SYSINFO, DESC, SCEN, CASE_ID, VALID} msgtype;
...
int addfilebytype(testlog *l, int fd, int dlevel);
/* This adds a file to the list of files being
   logged to. This adds by file descriptor
*/

int addfilebyname(testlog *l, const char* fname, int loglevel); 
/* This adds a file to the list of files being
...
int case_scenario(testlog *l, const char* scenario);
/* Logs a scenario name
 */

int case_info(testlog *l, const char* str);
/* Logs a description of the test case
 */

int case_description(testlog *l, const char *str);
/* Akin to info, but maps to the description field
in db/results reporting 
*/
...
int case_void(testlog *l, const char *vstr);
/* Specify the validation criteria for a test case
 */
...
int start(testlog *l, const char *msg);
