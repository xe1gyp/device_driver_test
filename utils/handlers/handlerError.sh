...
if [ "$LOCAL_OPERATION" = "clean" ]; then
	...
	test -d $HE_ERROR_FILE_HALT || rm -f $HE_ERROR_FILE_HALT
	test -d $HE_ERROR_FILE_CONTINUE || rm -f $HE_ERROR_FILE_CONTINUE
  ...
  if [ -f $HE_ERROR_FILE_HALT ]; then
    ...
    echo -e "\nFATAL: Error flag is set poiting to $LOCAL_HANDLER and will not continue\n"
    return 1
