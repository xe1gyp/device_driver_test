...
static int streaming_video(int od, int *fd,
	int sleep_time, int buf_count, const char *fname,
	int loop_count)
{
	struct {
		void *start;
...
exit:
	...
	if (--loop_count) {
		flag = 1;
		read_count = -1; /* reset the read count */
		goto repeat;
	}
end:
	return 0;
...
{
	printf("Usage: streaming_repeat <video_device> <inputfile> \
			[<n_sleep>] [<n_bufcount>] [<loop_count>]\n");
	return 1;
}
...
int main(int argc, char *argv[])
{
	...
	int sleep_time = 0, buf_count = 2, loop_count = 1000;
	...
	result = streaming_video(output_device, &file_descriptor,
		sleep_time, buf_count, argv[2], loop_count);
err1:
	close(output_device);
err2:
