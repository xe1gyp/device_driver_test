...
{
	#IN_FILE_NAME=`echo "sin-$SIN_FREQ-$1-$2-$3-$4"|tr - _`
	IN_FILE_NAME=`echo "sin_1000_$SIN_FREQ_$1_$2_$3_$4"`
	#OUT_FILE_NAME=`echo "$TMPBASE/REC-$SIN_FREQ-$1-$2-$3-$4.$$"|tr - _`
	OUT_FILE_NAME=`echo "$TMPBASE/REC_1000_$SIN_FREQ_$1_$2_$3_$4.$$"`
	OPT="-f $1"
	if [ "$2" = "24" ]; then
		OPT="$OPT -s 32"
	else
	...
	do	
		...
		$FD $FD_GENERIC_OPTION $OPT -i $RAWDIR/$IN_FILE_NAME -o $OUT_FILE_NAME -b $buffer
		COUNT=`expr $COUNT + 1`
		ERR=$?
		if [ $ERR -ne 0 ]; then
...
for insource in $INPUT_DEVICES
do
for outsource in $OUTPUT_DEVICES
do
	$CHANGER $CHANGER_GENERIC_OPTION -o $outsource -i $insource
