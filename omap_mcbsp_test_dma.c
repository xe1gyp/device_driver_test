...
static struct proc_dir_entry  *mcbsp_test_dir, *transmission_file,
		*status_file;
...
create_proc_file_entries(void)
{
	...
	if (!(status_file =
		create_proc_entry("status", 0644, mcbsp_test_dir)))
		goto no_status;
...
{
	printk(KERN_INFO "\nMCBSP_TEST: in %s  %d\n",
		__func__, mcbsptest_info[0].send_cnt++);
...
void omap_mcbsp_recv_cb(unsigned short int ch_status, void *arg)
{
	...
	if (no_of_rx <  no_of_trans) {
		if (0 != omap2_mcbsp_receive_data(mcbsptest_info[0].mcbsp_id,
					&mcbsptest_info[0],
					mcbsptest_info[0].rx_buf_dma_phys,
					buffer_size)) {
			printk(KERN_ERR "McBSP Test Driver:
					 Slave Send data failedn");
			return;
		}
...
void omap_mcbsp_recv_cb2(unsigned short int ch_status, void *arg)
{
	...
	if (no_of_rx2 <  no_of_trans) {
		if (0 != omap2_mcbsp_receive_data(mcbsptest_info[1].mcbsp_id,
					&mcbsptest_info[1],
...
int configure_mcbsp_tx(void)
{
	...
	/* XRST Reset */
	if (0 != omap2_mcbsp_set_xrst(mcbsptest_info[0].mcbsp_id,
		OMAP_MCBSP_XRST_DISABLE)) {
		printk(KERN_ERR "McBSP Test Driver: TX RST failed\n");
		return -1;
	}

	if (0 != omap2_mcbsp_set_xrst(mcbsptest_info[1].mcbsp_id,
		OMAP_MCBSP_XRST_DISABLE)) {
		printk(KERN_ERR "McBSP Test Driver: TX RST failed\n");
		return -1;
	}
	...
	if (0 != omap2_mcbsp_dma_trans_params(mcbsptest_info[0].mcbsp_id,
		&tp)) {
		printk(KERN_ERR "McBSP Test Driver:
	...
	mcbsptest_info[0].tx_buf_dma_virt = (void *)
		...
		&mcbsptest_info[0].tx_buf_dma_phys, GFP_KERNEL | GFP_DMA);
		tp.callback	= &omap_mcbsp_send_cb2;

	if (0 != omap2_mcbsp_dma_trans_params(mcbsptest_info[1].mcbsp_id,
		&tp)) {
		printk(KERN_ERR "McBSP Test Driver:
		Configuring transfer params failed\n");
		return -1;
	...
	mcbsptest_info[1].tx_buf_dma_virt = (void *)
		dma_alloc_coherent(NULL, buffer_size,
		&mcbsptest_info[1].tx_buf_dma_phys, GFP_KERNEL | GFP_DMA);
...
int configure_mcbsp_rx(void)
{
	...
	/* RRST */
	if (0 != omap2_mcbsp_set_rrst(mcbsptest_info[0].mcbsp_id,
		OMAP_MCBSP_RRST_DISABLE)) {
		printk(KERN_INFO "\n Receiver Reset failed \n");
	}

	if (0 != omap2_mcbsp_set_rrst(mcbsptest_info[1].mcbsp_id,
		OMAP_MCBSP_RRST_DISABLE)) {
		printk(KERN_INFO "\n Receiver Reset failed \n");
	}
	...
	mcbsptest_info[0].rx_buf_dma_virt = (void *)
		dma_alloc_coherent(NULL, buffer_size,
		&mcbsptest_info[0].rx_buf_dma_phys, GFP_KERNEL | GFP_DMA);
	...
	mcbsptest_info[1].rx_buf_dma_virt = (void *)
		dma_alloc_coherent(NULL, buffer_size,
		&mcbsptest_info[1].rx_buf_dma_phys, GFP_KERNEL | GFP_DMA);
	return 0;
}
...
int start_mcbsp_transmission(id)
{
	configure_mcbsp_tx();
	configure_mcbsp_rx();
	omap2_mcbsp_params_cfg(mcbsptest_info[id].mcbsp_id,
		OMAP_MCBSP_MASTER, &tp1, &rp1, &cfg);
	...
	printk(KERN_INFO "\n Start McBSP Receiver \n");
		/* RRST */
	if (0 != omap2_mcbsp_set_rrst(mcbsptest_info[id].mcbsp_id,
	...
	/* XRST */
	if (0 != omap2_mcbsp_set_xrst(mcbsptest_info[id].mcbsp_id,
		OMAP_MCBSP_XRST_ENABLE)) {
		printk(KERN_INFO "\n Unable to start transmitter \n");
	}

	printk(KERN_INFO " Start DMA Rx \n");
	if (0 != omap2_mcbsp_receive_data(mcbsptest_info[id].mcbsp_id,
					&mcbsptest_info[id],
	...
	printk(KERN_INFO " Start DMA Tx \n");
	if (0 != omap2_mcbsp_send_data(mcbsptest_info[id].mcbsp_id,
				&mcbsptest_info[id],
...
static
int __init omap2_mcbsp_init_dma(void)
{
...
static
void __exit omap_mcbsp_exit_dma(void)
{
	if (test_mcbsp_smp) {
	#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 00))
	consistent_free((void *)mcbsptest_info[1].rx_buf_dma_virt, buffer_size,
				mcbsptest_info[1].rx_buf_dma_phys);
	consistent_free((void *)mcbsptest_info[1].tx_buf_dma_virt, buffer_size,
				mcbsptest_info[1].tx_buf_dma_phys);
	#else
