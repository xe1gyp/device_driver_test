...
#define MAX_LIMIT_INTERACTIONS 200
...
int main (int argc, char **argv)
{
	...
	while (counter < MAX_LIMIT_INTERACTIONS) {
		...
		for (i = 0; i < rd / sizeof(struct input_event); i++)
			...
			if (ev[i].type == EV_SYN) {
				printf("Count: %d, Event: time %ld.%06ld, -------------- %s ------------\n",
					counter,
          ev[i].time.tv_sec, ev[i].time.tv_usec, ev[i].code ? "Config Sync" : "Report Sync" );
			} else if (ev[i].type == EV_MSC && ev[i].code == MSC_RAW) {
				printf("Count: %d, Event: time %ld.%06ld, type %d (%s), code %d (%s), value %02x\n",
					counter,
          ev[i].time.tv_sec, ev[i].time.tv_usec, ev[i].type,
				...
				printf("Count: %d, Event: time %ld.%06ld, type %d (%s), code %d (%s), value %d\n",
				  counter,
					ev[i].time.tv_sec, ev[i].time.tv_usec, ev[i].type,
					events[ev[i].type] ? events[ev[i].type] : "?",
					ev[i].code,
