...
if [ "$LOCAL_COMMAND" = "create" ]; then
	...
	if [ "$LOCAL_DRIVER" = "file_storage" ]; then
		...
		echo "Inserting the module"	
			...
			echo "Command: dd if=/dev/urandom of=/mnt/ramdrive/usb.ext2.temporal.file bs=1M count=20"
			dd if=/dev/urandom of=/mnt/ramdrive/usb.ext2.temporal.file bs=1M count=20; sync
