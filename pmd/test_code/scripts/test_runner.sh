...
usage()
{
    ...
    -p              Human readable(dont laugh too much) format logfiles.
                    [Current - ($PP)]
    -z              Dont Merge Scenario Name with tcid to create final tcid
    -E              Use Extended Test cases also - these are painful and can
                    take real long time
...
sanity_check()
{
    ...
    [ ! -d "$TMPBASE" -o ! -w "$TMPBASE" ] && \
die "$TMPBASE - cannot work as temporary directory"

    [ ! -d "$TESTBIN" ] &&\
 die "$TESTBIN - cannot find test binary directory"
    [ ! -d "$TESTDIR" -o ! -w "$TESTDIR" ] && \
die "$TESTDIR - cannot work as test directory"
    [ ! -d "$TC_SCENARIO" ] && \
die "$TC_SCENARIO - No such directories"
    [ -z "$SCENARIO_NAMES" ] && die "No Scenarios"
    #[ ! -z "$VERBOSE" -a ! -z "$QUIET_MODE" ] && \
#die "Make up your mind - verbose or quiet??"
    export CMDFILE=$TMPBASE/$CMDFILE
    rm -f $CMDFILE
    for SCEN in $SCENARIO_NAMES
    do
        [ ! -f "$TC_SCENARIO/$SCEN" -o ! -r "$TC_SCENARIO/$SCEN" ] && \
die "$TC_SCENARIO/$SCEN - not a scenario file"
        cat $TC_SCENARIO/$SCEN|grep -v "#"|sed -e "s/^[  ]*$//g"|\
sed -e "/^$/d">$TMPFILE|| die "Count not create tmp file $TMPFILE"
	if [ -z "$DONT" ]; then
 ...
 die "Count not create command file $CMDFILE"
	...
	if [ -z "$EXTENDED_TEST" ]; then
		...
		cat $CMDFILE|grep -v "^[_A-Za-z0-9]*_EXT ">$TMPFILE ||\
 die "intermediate file gen failed"
		cat $TMPFILE>$CMDFILE || die "Second intermediate \
creation failed"
	fi
	if [ -z "$INTERACTIVE" ]; then
		#Remove the test cases which require user interaction
		cat $CMDFILE|grep -v "^[_A-Za-z0-9]*_INT ">$TMPFILE ||\
 die "intermediate file gen failed"
		cat $TMPFILE>$CMDFILE || die "Second intermediate \
creation failed"
	fi
...
main()
{
   count=0
    ...
    # run pan
    #$PAN_COMMAND #Duplicated code here, because otherwise
    # if we fail, only "PAN_COMMAND" gets output
        #Usage: pan -n name [ -SyAehp ] [ -s starts ]
        # [-t time[s|m|h|d] [ -x nactive ] [
        #-l logfile ]
...
cleanup()
{
    ...
    if [ -n "${TMPFILE}" -a -n "${CMDFILE}" -a -n \
"${TESTDIR}" -a -n "${TMPBASE}" ]; then
        rm -rf ${TMPFILE} ${CMDFILE} ${TESTDIR}/* ${TMPBASE}/*
    else
        echo "INFO: Clean up process won't be executed
 because variables for directories to be removed are not set..."
    fi
