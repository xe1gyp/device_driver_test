...
#
# Fix:
# -May 08 2007-  Modified for 2430 8x/18x to read the L3 clock and set the valid Operating point
...
#
# The test uses the fast operating point as its default sleep instead of the current
#  this means you will wake up fast, even if you went in as slow.
# 
...
#                     PRCM Volt DPLLM DPLLN CORE ARMDIV SLEEP
	...
	if [ $BMips -eq 88 -o $BMips -eq 176 ]; then
...
#                    PRCM Volt DPLLM DPLLN CORE ARMDIV SLEEP
	...
	# Read BogoMips and find the valid operating point set
#	BMips=` cat /proc/cpuinfo |grep Bogo|cut -d ':' -f 2|cut -d '.' -f1`
#   Modified to read the L3 and find the valid operating point set
   
    L3ck=`  cat /proc/ckomap24xx | grep l3 | cut -d ' ' -f2 | cut -c 1-3`
    if [ $L3ck -gt 133 ]; then 
