...
case "$TESTCASEID" in
	...
	4)
		verifyspeed.sh $MODE || exit 2 && for i in $I2C_ADDRESSES; do i2cdump -f -y $I2C_ADAPTER_1 $i b || exit 1; done
		verifyspeed.sh $MODE || exit 2 && ((testrw.sh $I2C_ADAPTER_1 $I2C_REG_1 $I2C_REG_1_VALUE_INITIAL -d 1 &) && testrw.sh $I2C_ADAPTER_1 $I2C_REG_2 $I2C_REG_2_VALUE_INITIAL -d 1 &) && sleep 1 && killall testrw.sh || exit 1
		verifyspeed.sh $MODE || exit 2 && for i in $I2C_ADDRESSES; do (i2cdump -f -d -y $I2C_ADAPTER_1 $i b &) && killall i2cdump || exit 1; done
		verifyspeed.sh $MODE || exit 2 && for i in $I2C_ADDRESSES; do i2cdump -f -y $I2C_ADAPTER_1 $i b || exit 1; done
		verifyspeed.sh && for i in $I2C_ADDRESSES; do i2cdump -f -y $I2C_ADAPTER_1 $i b || exit 1; done
