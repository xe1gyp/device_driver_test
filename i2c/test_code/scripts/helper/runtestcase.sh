...
case "$TESTCASEID" in
	...
	1)
		verifyspeed.sh $MODE || exit 2 && for i in $I2C_ADDRESSES; do i2cdump -y $I2C_ADAPTER_1 $i b || exit 1; done
	...
	4)
		...
		verifyspeed.sh $MODE || exit 2 && for i in $I2C_ADDRESSES; do (i2cdump -y $I2C_ADAPTER_1 $i b &) && killall i2cdump || exit 1; done
-		verifyspeed.sh $MODE || exit 2 && for i in $I2C_ADDRESSES; do i2cdump -y $I2C_ADAPTER_1 $i b || exit 1; done
		verifyspeed.sh && for i in $I2C_ADDRESSES; do i2cdump -y $I2C_ADAPTER_1 $i b || exit 1; done
