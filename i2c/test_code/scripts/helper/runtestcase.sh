...
case "$TESTCASEID" in
	...
	4)
		verifyspeed.sh $MODE || exit 1 && for i in $I2C_ADDRESSES; do i2cdump -y $I2C_ADAPTER_1 $i b || exit 1; done
		verifyspeed.sh $MODE || exit 1 && ((testrw.sh $I2C_ADAPTER_1 $I2C_REG_1 $I2C_REG_1_VALUE_INITIAL &) && testrw.sh $I2C_ADAPTER_1 $I2C_REG_2 $I2C_REG_2_VALUE_INITIAL &) && sleep 1 && killall testrw.sh || exit 1
	...
	5)
		verifyspeed.sh $MODE || exit 1 && for i in $I2C_ADDRESSES; do i2cdump -y $I2C_ADAPTER_1 $i b || exit 1; done
		verifyspeed.sh $MODE || exit 1 && echo `cat $PROCFS_INTERRUPTS | grep $INTERRUPT_PREFIX | grep $INT_24XX_I2C1_IRQ: | sed -e "s/ */ /g" | cut -d ' ' -f3` > $TEMP_FILE_1 || exit 1
		verifyspeed.sh $MODE || exit 1 && for i in $I2C_ADDRESSES; do i2cdump -y $I2C_ADAPTER_1 $i b || exit 1; done
		verifyspeed.sh $MODE || exit 1 && echo `cat $PROCFS_INTERRUPTS | grep $INTERRUPT_PREFIX | grep $INT_24XX_I2C1_IRQ: | sed -e "s/ */ /g" | cut -d ' ' -f3` > $TEMP_FILE_2 || exit 1
