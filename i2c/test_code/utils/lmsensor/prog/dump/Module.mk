...
PROGDUMPMAN8FILES := $(MODULE_DIR)/i2cdump.8 $(MODULE_DIR)/i2cset.8/ \
				$(MODULE_DIR)/i2c_master_test.8 $(MODULE_DIR)/i2c_slave_test.8

# Regrettably, even 'simply expanded variables' will not put their currently
# defined value verbatim into the command-list of rules...
PROGDUMPTARGETS := $(MODULE_DIR)/i2cdump $(MODULE_DIR)/i2cset \
			$(MODULE_DIR)/i2c_master_test $(MODULE_DIR)/i2c_slave_test
PROGDUMPSOURCES := $(MODULE_DIR)/i2cdump.c $(MODULE_DIR)/i2cset.c \
		   $(MODULE_DIR)/i2cbusses.c $(MODULE_DIR)/i2c_master_test.c $(MODULE_DIR)/i2c_slave_test.c
...
$(MODULE_DIR)/i2c_master_test: $(MODULE_DIR)/i2c_master_test.ro $(MODULE_DIR)/i2cbusses.ro
	$(CC) $(EXLDFLAGS) -o $@ $^

$(MODULE_DIR)/i2c_slave_test: $(MODULE_DIR)/i2c_slave_test.ro $(MODULE_DIR)/i2cbusses.ro
	$(CC) $(EXLDFLAGS) -o $@ $^
