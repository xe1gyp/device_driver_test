...
{
	fprintf(stderr,
		"Syntax: i2cdump [-y] [-h] [-t] [-i] I2CBUS ADDRESS [MODE] [BANK "
		"[BANKREG]]\n" "        i2cdump -V\n"
		"  With -h, uses High speed mode (NOT RECOMMENDED)\n"
		"  With -t, uses Ten Bit addressing mode (NOT RECOMMENDED)\n"
		"  With -i, uses Raw i2c calls to do transfer not smbus apis\n"
...
static int readb_i2c_bus(int file, int i2c, int cmd)
{
	...
	if (!i2c) {
		if (cmd)
			res = i2c_smbus_read_byte_data(file, cmd);
		else
			res = i2c_smbus_read_byte(file);
	} else {
		/* Write reg addr first */
		res = write(file, &data, 1);
		if (res < 0)
			return res;
		/* now request data */
...
int main(int argc, char *argv[])
{
	...
	int pec = 0;
	int flags = 0;
	int yes = 0, version = 0, hs = 0, ten = 0, i2c = 0;
	...
	if (hs) {
		...
		printf("HS MODE:");
		if (ioctl(file, I2C_HIGH_SPEED, 1) < 0) {
			printf("FAILED\n");
	...
	if (ten) {
		printf("TEN BIT ADDRESSING MODE:");
		if (ioctl(file, I2C_TENBIT, 1) < 0) {
			printf("FAILED\n");
			fprintf(stderr,
				"Error: Could not set Ten Bit Mode %s\n",
				strerror(errno));
			exit(1);
		}
		printf("OK\n");
	...
	if (size != I2C_SMBUS_WORD_DATA) {
		...
		if (size == I2C_SMBUS_BLOCK_DATA
			...
			if (size == I2C_SMBUS_BLOCK_DATA) {
				...
				for (i = 0; i < 256; i += 32) {
					res2 =
					    i2c_smbus_read_i2c_block_data(file,
									  i,
									  cblock
									  + i);
					if (res2 <= 0)
		...
		for (i = 0; i < 256; i += 16) {
			...
			for (j = 0; j < 16; j++) {
				if (size == I2C_SMBUS_BYTE_DATA) {
					block[i + j] = res =
					    readb_i2c_bus(file, i2c, i + j);
				} else if (size == I2C_SMBUS_BYTE) {
					block[i + j] = res =
					    readb_i2c_bus(file, i2c, 0);
				} else
					res = block[i + j];
				if (res < 0)
					printf("XX ");
