...
#include <asm-arm/arch-omap/prcm_34xx.h>
#include <asm-arm/arch-omap/prcm-debug.h>
...
int co_test_init(void)
{
	 ...
	 if (co_type == 0x6 && target_val == 1 && preempt == 1)
        {
			...
			Con_Threads[i] = kthread_create((void *)kernel_preemption_test,
	...
	if (co_type >= 0x1 && co_type < 0x6 && preempt == 0 ) {
		...
		thread_input_ptr  = constraint_test(map_table[co_type-1], target_val,
								REQUEST_CO,thread_input_ptr);
	}
	return 0;
}
